...
RSpec.describe "CLI" do
  ...
  context "with command" do
    ...
    it "with required_argument" do
      output = capture_output { cli.call(arguments: %w[first_arg --mandatory-option=required]) }
    ...
    it "with optional_arg" do
      output = capture_output { cli.call(arguments: %w[first_arg opt_arg --mandatory-option=required]) }
      ...
      if RUBY_VERSION < "3.4"
        expect(output).to eq(
          ...
          "mandatory_option: required. " \
          "Options: {:option_with_default=>\"test\", :mandatory_option_with_default=>\"mandatory default\", " \
          ":mandatory_option=>\"required\", :args=>[\"opt_arg\"]}\n"
        )
      else
        expect(output).to eq(
          "mandatory_arg: first_arg. optional_arg: opt_arg. " \
    ...
    it "with underscored option_one" do
      output = capture_output { cli.call(arguments: %w[first_arg --option_one=test2 --mandatory-option=required]) }
    ...
    it "with option_one alias" do
      output = capture_output { cli.call(arguments: %w[first_arg -1 test2 --mandatory-option=required]) }
    ...
    it "with underscored boolean_option" do
      output = capture_output { cli.call(arguments: %w[first_arg --boolean_option --mandatory-option=required]) }
    ...
    it "with boolean_option alias" do
      output = capture_output { cli.call(arguments: %w[first_arg -b --mandatory-option=required]) }
    ...
    it "with underscoreed option_with_default alias" do
      output = capture_output do
        cli.call(arguments: %w[first_arg --option_with_default=test3 --mandatory-option=required])
      end
    ...
    it "with combination of aliases" do
      output = capture_output { cli.call(arguments: %w[first_arg -bd test3 --mandatory-option=required]) }
