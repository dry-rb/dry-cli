  ...
  context "works with params" do
    ...
    context "with required params" do
      ...
      it "more than one param and with optional params" do
        output = capture_output { cli.call(arguments: %w[generate action web users#index --url=/signin]) } # rubocop:disable Layout/LineLength
      ...
      it "more than one param and with boolean params" do
        output = capture_output { cli.call(arguments: %w[generate action web users#index --skip-view --url=/signin]) } # rubocop:disable Layout/LineLength
      ...
      it "more than required params" do
        output = capture_output { cli.call(arguments: %w[destroy action web users#index unexpected_param]) } # rubocop:disable Layout/LineLength
        expect(output).to eq("destroy action - app: web, action: users#index\n")
      end
